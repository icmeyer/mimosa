...
lattice = 9
fuel_refine_level = 4
guide_locations = np.array([18,21,32,54,57,60,72,75,78])
ngroup = 10
pitch = 1.26
length = 1.26*lattice
...
for i in range(lattice*lattice):
    ...
    for level in range(fuel_refine_level - 1):
        fuel = Region([surfaces[outer+level], surfaces[outer+level+1]], [-1, 1], uid=region_counter, mat=fill,phi=fuel_phi_guess)
        region_counter += 1
        regions.append(fuel)
        sub_regions.append(fuel)
    # Make donut hole
    fuel = Region([surfaces[inner]], [-1], uid=region_counter, mat=fill,phi=fuel_phi_guess)
    region_counter += 1
    regions.append(fuel)
    sub_regions.append(fuel)
